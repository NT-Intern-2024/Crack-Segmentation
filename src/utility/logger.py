...
from .project import change_to_main_root


def set_logger_config():
    logging_format = "%(asctime)s - %(message)s"
    logging.basicConfig(
        filename="all-process.log", format=logging_format, level=logging.INFO
    )


def setup_logger(
    name: str, log_file: str, level=logging.INFO, logging_format: str = None
):
...
logger_backtrack = setup_logger(
    "Backtrack", "backtrack.log", logging_format=simple_log_format
)
